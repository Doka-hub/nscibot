    ...
    if callback:
        ...
        if image_id:
            ...
            if callback.message.content_type in ['photo', 'video']:
                await callback.message.edit_caption(text_answer, reply_markup=mail_create_inline_keyboard, parse_mode='markdown')
            else:
                await callback.message.edit_text(text_answer, reply_markup=mail_create_inline_keyboard, parse_mode='markdown')
    elif message:
